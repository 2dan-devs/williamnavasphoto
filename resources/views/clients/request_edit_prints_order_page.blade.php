<script>
	...
	var albumID = {{$albumID}};
    var orderID = {{$orderID}};
    formats = JSON.parse(replaceAll("&quot;","\"",'{{$photoFormats}}'));
    selections = JSON.parse(replaceAll("&quot;","\"",'{{$selections}}'));
	var DOMAINNAME = "{{ url().'/' }}";
	...
	*/
	...
	function addHTMLPhoto(photo,photoIndex,totalPhotos){
		var photoData = {
			photoID: photo.id,
			...
			storageEngine.findByProperty("photo_prints","photo_id",photo.id.toString(),function(result){
				...
				//add clients selected formats and quatities with delete button
				...
				htmloutput +=				'<input type="text" class="picture-quantity" onkeypress="return isNumber(event)" value="1">';
				htmloutput +=			'</div>';
				htmloutput +=			'<div class="medium-1 large-1 columns arrow-container">';
